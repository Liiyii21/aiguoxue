...
assert.match(viteSource, /divination/, "divination project should open the divination page at the root URL");
assert.match(envSource, /VITE_PAGE_ID=divination/, "divination local env should open the divination standalone page");
assert.match(supabaseConfigSource, /window\.__SUPABASE_CONFIG__/, "divination runtime Supabase config should be loaded before the app boots");
assert.match(supabaseConfigSource, /zahnteyhzrwqjgdgmmjz\.supabase\.co/, "divination runtime Supabase config should point at the shared project");
...
assert.match(appSource, /analyzeDivinationReport/, "divination actions should call the model-backed divination analysis API");
assert.match(appSource, /DivinationAiAnalysisPanel/, "divination pages should render AI divination analysis results");
assert.match(appSource, /divinationAiAnalysis/, "divination pages should keep the latest AI divination analysis in state");
assert.match(appSource, /topicNotes/, "divination quick topic cards should open structured detail content");
assert.match(appSource, /actionByStep = \[null, "spin", "share", "consult"/, "divination flow cards should trigger the matching actions");
assert.match(appSource, /title: "当前判断"/, "divination topic detail should expose a visible section");
assert.doesNotMatch(appSource, /analysis\.model/, "divination analysis cards should not expose the model name in the UI");
assert.doesNotMatch(appSource, /本地兜底/, "divination analysis cards should not expose provider fallback labels in the UI");
...
assert.match(styleSource, /\.divination-screen \.fortune-card/, "divination report card polish should exist");
...
assert.match(directusSource, /export async function analyzeDivinationReport/, "Directus API layer should expose divination AI analysis");
assert.match(directusSource, /\/ai\/divination\/analyze/, "Directus API layer should call the divination analysis API");
...
assert.match(mockDirectusSource, /url\.pathname === "\/ai\/divination\/analyze"/, "local mock Directus should support divination AI analysis");
assert.match(mockDirectusSource, /buildDivinationAiAnalysis/, "local mock Directus should build divination AI analysis");
...
assert.match(mockDirectusSource, /max_output_tokens:\s*\d+/, "text model responses calls should cap output tokens to avoid provider timeouts");
assert.match(mockDirectusSource, /max_tokens:\s*\d+/, "text model chat fallback should cap output tokens to avoid provider timeouts");
assert.match(mockDirectusSource, /cultural self-reflection content/, "divination AI prompt should use provider-stable cultural self-reflection wording");
assert.doesNotMatch(mockDirectusSource, /divination-style content assistant/, "divination AI prompt should avoid provider-timeout-prone divination wording");
assert.match(mockDirectusSource, /timeoutMs\s*=\s*30000/, "text model provider requests should allow enough time for slow-but-successful model responses");
assert.match(mockDirectusSource, /preferChat/, "text model provider should support chat-first mode for endpoints where responses is slow");
assert.match(mockDirectusSource, /buildDivinationAiAnalysis[\s\S]*preferChat:\s*true/, "divination AI should use chat-first provider mode");
